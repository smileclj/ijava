 ...
 * $Id: SecurityUtils.java 28839 2012-07-06 04:51:49Z huangwq $
 */
package util;
 ...
 * @author liangxiao
 * @author huangwj
 * @version $Revision: 28839 $, $Date: 2012-07-06 12:51:49 +0800 (周五, 06 七月 2012) $
 */
public abstract class SecurityUtils {
    ...
    /**
     * 自身混淆加密，最多只能加密 30 个字节长度的字符串。<font color='red'>只能加密非中文的字符串</font>
     * 
     * <p>
    ...
    public static String encodeBySHA1(String str) {
        try {
            ...
            md.update(StringUtils.getBytesUTF8(str));
    ...
    private static SecretKeySpec getSecretKeySpec(String key) throws NoSuchAlgorithmException {
        ...
        kgen.init(128, new SecureRandom(StringUtils.getBytesUTF8(key)));
        SecretKey secretKey = kgen.generateKey();
        byte[] enCodeFormat = secretKey.getEncoded();
        // SecretKeySpec keySpec = new SecretKeySpec(enCodeFormat, "AES");
        SecretKeySpec keySpec = new SecretKeySpec(StringUtils.getBytesUTF8(key), "AES");
        return keySpec;
    }

    /**
     * 使用 AES 加密，然后使用 Base64 编码。,默认为AES/ECB/PKCS5Padding模式,只支持AES128，即16位密钥，如果密钥不足16位，用0补足，如果超过16位，则截取前16位
     * 
     * @param content
     *            需要加密的内容
     * @param password
     ...
     * @return
     */
    public static String encodeByAesAndBase64(String content, String key) {
        key = key.substring(0, 16);
        String ret = null;
        try {
            SecretKeySpec keySpec = getSecretKeySpec(key);
            Cipher cipher = Cipher.getInstance("AES/ECB/PKCS5Padding");// 创建密码器,默认为AES/ECB/PKCS5Padding模式
            byte[] byteContent = StringUtils.getBytesUTF8(content);

            // IvParameterSpec iv = new IvParameterSpec("0123456789123456".getBytes());// 使用CBC模式，需要一个向量iv，可增加加密算法的强度
            // cipher.init(Cipher.ENCRYPT_MODE, keySpec, iv);// 初始化

            cipher.init(Cipher.ENCRYPT_MODE, keySpec);// 初始化
            byte[] result = cipher.doFinal(byteContent);
            ret = StringUtils.newStringUTF8(Base64.encodeBase64(result, false));
        }
        catch (Exception e) {
            throw new RuntimeException("Could not encodeByAesAndBase64", e);
        }
        return ret;
    }
    ...
    public static void main(String[] args) {
        // String key = "新风作浪";
        // key = "111111111111111u";
        // String content = "不会吧";
        // System.out.println(encodeByAesAndBase64(content, key));
        // System.out.println(decodeByAesAndBase64(encodeByAesAndBase64(content, key), key));
        String r = SecurityUtils.encodeByAesAndBase64("1312312", "4028810c42988a9c");
        System.out.println(r);// J2WPtZ7D1UELPSDXrxgeFw==
        System.out.println(SecurityUtils.decodeByAesAndBase64("J2WPtZ7D1UELPSDXrxgeFw==", "4028810c42988a9c"));

        System.out.println(SecurityUtils.decodeByAesAndBase64("MV5onZpZSzLCOtK/mcYvqg==", "4028810c42988a9c"));
    }

    /**
     * 使用 Base64 解码，然后使用 AES 解密。,默认为AES/ECB/PKCS5Padding模式,只支持AES128，即16位密钥，如果密钥不足16位，用0补足，如果超过16位，则截取前16位
     * 
     * @param content
     *            待解密内容
     * @param key
     ...
     * @return
     */
    public static String decodeByAesAndBase64(String content, String key) {
        key = key.substring(0, 16);
        String ret = null;
        try {
            SecretKeySpec keySpec = getSecretKeySpec(key);
            ...
            byte[] result = cipher.doFinal(new Base64().decode(StringUtils.getBytesUTF8(content)));
            ret = StringUtils.newStringUTF8(result);
        }
        catch (Exception e) {
            throw new RuntimeException("Could not decodeByAesAndBase64", e);
        }
        return ret;
    }
    ...
    public static String decodeBy36Radix(String str) {
        ...
        return new String(bytes, 0, offset, Charset.forName("UTF8"));
    }
